...
## The technical spec defines the public Bartl boundary

The narrow technical contract lives in `src/content/spec.ts` and is rendered by `src/pages/[lang]/spec.astro`. It defines Bartl as an observed-state startup mechanism for a stateful application. The landing page summarizes that same contract and links to the executable WordPress reference.
...
The landing route renders `src/components/Landing.astro` for both languages. Landing and spec copy live in `src/content/landing.ts` and `src/content/spec.ts`; each page keeps one shared layout across languages. Copy remains language-specific, but section order, links, semantics, and claim boundaries cannot drift through separate page implementations.
